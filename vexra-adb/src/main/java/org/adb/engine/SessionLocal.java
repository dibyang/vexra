...
import org.adb.command.Command;
import org.adb.command.CommandInterface;
import org.adb.command.Parser;
...
import org.adb.index.QueryExpressionIndex;
import org.adb.jdbc.JdbcConnection;
import org.adb.jdbc.meta.DatabaseMeta;
import org.adb.jdbc.meta.DatabaseMetaLocal;
...
import org.adb.mvstore.db.MVIndex;
import org.adb.mvstore.db.MVTable;
...
import org.adb.store.LobStorageFrontend;
...
public final class SessionLocal extends Session implements TransactionStore.RollbackListener {
    ...
    if (localTempTables != null && !localTempTables.isEmpty()) {
      ...
      while (it.hasNext()) {
        ...
        if (closeSession || table.getOnCommitDrop()) {
          ...
          table.setModified();
          it.remove();
          // Exception thrown in org.adb.engine.Database.removeMeta
   ...
   *
   * @param derivedTable true to get the cache of derived tables
  ...
  public Transaction getTransaction() {
    if (transaction == null) {
      ...
      TxnManager txnManager = database.getTxnManager();
      if(txnManager!=null){
        Transaction2 transaction2 = txnManager.beginTransaction();
        transaction.setTxnMap2(new TxnMap2(txnManager, transaction2));
      }
      startStatement = -1;
    }
    return transaction;
