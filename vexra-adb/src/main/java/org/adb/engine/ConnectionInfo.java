...
import org.adb.store.fs.encrypt.FilePathEncrypt;
import org.adb.store.fs.rec.FilePathRec;
import org.adb.util.IOUtils;
import org.adb.util.NetworkConnectionInfo;
import org.adb.util.SortedProperties;
...
public class ConnectionInfo implements Cloneable {
  ...
  public static final String HA2_KEY = "?ha2=";
  ...
  //db模式
  private DbStoreType storeType = DbStoreType.NONE;
  ...
  public static void main(String[] args) {
    String url = "jdbc:adb:tcp:13.13.14.141:9002//ha2/b_db?ha2=/etc/b_db.cfg";
    ConnectionInfo info = new ConnectionInfo(url,new Properties(),"","");
    ...
    if (idx < 0) {
      throw info.getFormatException();
    }
    String databaseName = name.substring(idx + 1);
    System.out.println("databaseName = " + databaseName);
    String server = name.substring(0, idx);
    System.out.println("server = " + server);
    ConnectionInfo info2 = new ConnectionInfo(databaseName);
    info2.setOriginalURL(info.getOriginalURL());
    System.out.println("info2.name = " + info2.name);
    System.out.println("info2.url = " + info2.url);
    System.out.println("info2.storeType = " + info2.storeType);
  }

  /**
   * Create a connection info object.
   *
   * @param u        the database URL (must start with jdbc:adb:)
  ...
  public ConnectionInfo(String u, Properties info, String user, Object password) {
    ...
    if (recoverTest != null) {
      ...
      try {
        Utils.callStaticMethod("org.adb.store.RecoverTester.init", recoverTest);
      } catch (Exception e) {
  ...
  public ConnectionInfo clone() throws CloneNotSupportedException {
    ...
    clone.info = (Properties) info.clone();
  ...
  private void parseName() {
    ...
    if (name.startsWith("tcp:")) {
      ...
      name = name.substring("rocks:".length());
      persistent = false;
      storeType = DbStoreType.ROCKSDB;
    } else if (name.startsWith("ldb:")) {
      name = name.substring("ldb:".length());
      persistent = false;
      storeType = DbStoreType.LDB;
    } else if (name.startsWith("ha2:")) {
      name = name.substring("ha2:".length());
      persistent = false;
      storeType = DbStoreType.HA2;
      int index = name.indexOf(HA2_KEY);
      if(index>0){
        String config = name.substring(index+ HA2_KEY.length());
        ...
        if(configPath.toFile().exists()) {
          ...
          try(InputStream in = Files.newInputStream(configPath)) {
            ...
            for (Object key : conf.keySet()) {
              info.put(key.toString().toUpperCase(), conf.get(key));
            }
          }catch (IOException e) {
            throw DbException.convert(e);
          }
  ...
  public DbStoreType getStoreType() {
    return storeType;
  }
  ...
  private void readProperties(Properties info) {
    ...
    for (Object k : list) {
      ...
      if (isKnownSetting(key)||key.startsWith("HA2.")) {
        prop.put(key, value);
      } else {
        if (s == null) {
          s = getDbSettings();
