...
import org.adb.util.ThreadDeadlockDetector;
...
public final class Engine {
        ...
        if (ci.getProperty("JMX", false)) {
            try {
                Utils.callStaticMethod(
                        "org.adb.jmx.DatabaseInfo.registerMBean", ci, database);
            } catch (Exception e) {
    ...
    static void close(String name) {
        if (JMX) {
            try {
                Utils.callStaticMethod("org.adb.jmx.DatabaseInfo.unregisterMBean", name);
            } catch (Exception e) {
                throw DbException.get(ErrorCode.FEATURE_NOT_SUPPORTED_1, e, "JMX");
            }
        }
