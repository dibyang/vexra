...
public class RowCountKey extends Key{
  ...
  RowCountKey(byte[] data) {
    ...
    if (this.metaType != MetaType.TABLE_ROW_COUNT) {
  ...
  public static RowCountKey fromBytes(byte[] data) {
    return new RowCountKey(data);
  }

  public static RowCountKey of(TabId tId) {
    DynamicByteBuffer b =  DynamicByteBuffer.c();
    b.put(KeyType.META.getCode());
    b.put(MetaType.TABLE_ROW_COUNT.getCode());
    b.putInt(tId.id);
    b.putLong(tId.epoch);
    return new RowCountKey(b.toArray());
  }
