...
import net.xdob.vexra.server.raftlog.segmented.CacheInvalidationPolicy.CacheInvalidationPolicyDefault;
import net.xdob.vexra.server.raftlog.segmented.LogSegment.LogRecord;
import net.xdob.vexra.util.AutoCloseableLock;
import net.xdob.vexra.util.AutoCloseableReadWriteLock;
...
public class SegmentedRaftLogCache {
  ...
  void evictCache(long[] followerIndices, long safeEvictIndex, long lastAppliedIndex) {
    List<LogSegment> toEvict = evictionPolicy.evict(followerIndices,
		...
		LOG.info("Evicting cache for toEvict: {}, ",toEvict);
