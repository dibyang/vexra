...
截至 2026-06-20，当前计划已经完成 `ADB-Runtime-01` 到 `ADB-Runtime-11`、生产化阶段 `ADB-Prod-01` 到 `ADB-Prod-06`，以及可运行化阶段 `ADB-Run-01` 到 `ADB-Run-12`。因此，按当前路线图统计，剩余需要完成的阶段数为 0 个。后续如果新增阶段，必须同步更新本快照、下方阶段表和阶段状态说明，并进行本地提交。
...
| Runnable Cluster Hardening 可运行化阶段 | 0 | `ADB-Run-01` 到 `ADB-Run-12` 已完成 | 见“Runnable Cluster Hardening 阶段” |

当前路线图已新增 5 个可运行化产品阶段，其中 `ADB-Run-12` 已完成。当前可运行化阶段已全部闭合；后续新增 TiDB-like 产品化目标时，应另起阶段并同步更新本节。
...
当前生产化路线图已经完成，`ADB-Run-*` 阶段专门追踪真实进程入口、启动命令、运行手册和端到端 smoke。当前规划 12 个可运行化阶段，`ADB-Run-01` 到 `ADB-Run-12` 均已完成，本组阶段剩余数为 0。

| 口径 | 数量 | 说明 |
| --- | --- | --- |
| 已完成可运行化阶段 | 12 | `ADB-Run-01` 已完成 main 包 ADB region node 产品入口验收；`ADB-Run-02` 已完成产品 main class OS 多进程 Raft/GRPC smoke；`ADB-Run-03` 已完成 SQL server 产品入口和 TCP/JDBC smoke；`ADB-Run-04` 已完成 runtime 发行包和双入口启动脚本；`ADB-Run-05` 已完成 runtime zip 解包后的脚本级 SQL/JDBC smoke；`ADB-Run-06` 已完成 runtime zip 解包后的 region node 脚本级多进程 Raft/GRPC smoke；`ADB-Run-07` 已完成 SQL server 远端 Raft region scan smoke；`ADB-Run-08` 已完成 SQL server 远端 region 写入 smoke；`ADB-Run-09` 已完成 SQL server 共享 catalog/TSO 原型；`ADB-Run-10` 已完成 SQL server 与 region node 自动编排原型；`ADB-Run-11` 已完成安装器与安全默认值模板；`ADB-Run-12` 已完成端到端集群压测门禁。 |
| 进行中可运行化阶段 | 0 | 当前没有进行中的 `ADB-Run-*` 阶段。 |
| 未开始可运行化阶段 | 0 | 当前没有未开始的 `ADB-Run-*` 阶段。 |
| 剩余需完成可运行化阶段 | 0 | 新增可运行化产品阶段已全部完成。 |
...
| 12 | ADB-Run-12 | 已完成 | 端到端集群压测门禁 | 长稳压测脚本、故障注入矩阵、发布报告 | 集群读写、故障恢复和滚动升级报告满足门禁 |

### ADB-Run-01 实施口径

`ADB-Run-01` 的目标是把 `ADB-Prod-01` 中测试专用的 `AdbRaftRegionServerProcess` 能力提升为 main 包可复用的节点入口：

- 新增 main 包 ADB region node 启动类，支持 `--group`、`--node`、`--peers`、`--host`、`--port`、`--storage`、`--cache`、`--ready` 和 `--stop` 参数。
- 新增可测试的启动配置对象和 RaftServer 工厂方法，复用 `AdbStateMachine`、GRPC RPC 类型、storage/cache 配置和现有 peer 解析。
- `--ready` 和 `--stop` 参数保留为可选运维钩子：本地 smoke 可以用 ready 文件确认启动完成，用 stop 文件优雅退出；生产环境也可以由外部进程管理器省略它们。
...
`ADB-Run-04` 的目标是让 main 包产品入口形成可分发、可执行的 runtime 产物：

- `vexra-adb` Gradle 构建新增 SQL server 和 region node 两套 `CreateStartScripts` 任务。
- 新增 runtime zip 任务，把 `vexra-adb` jar、runtimeClasspath 依赖和两套启动脚本打包到 `lib/` 与 `bin/`。
- 启动脚本只负责把 classpath 与 main class 固化到发行包，不内置生产参数、TLS、认证或服务编排。
- 阶段验收要求 `:vexra-adb:adbRuntimeDist` 能生成 zip，zip 内包含 `bin/adb-sql-server`、`bin/adb-region-node` 和 `lib/vexra-adb-*.jar`。

`ADB-Run-04` 已完成：

- `vexra-adb` 新增 `adbSqlServerStartScripts`、`adbRegionNodeStartScripts` 和 `adbRuntimeDist` Gradle 任务。
- `:vexra-adb:adbRuntimeDist` 已生成 `vexra-adb-0.1.0-SNAPSHOT-runtime.zip`。
- zip 内容已验证包含 `bin/adb-sql-server`、`bin/adb-sql-server.bat`、`bin/adb-region-node`、`bin/adb-region-node.bat` 和 `lib/vexra-adb-0.1.0-SNAPSHOT.jar`。

### ADB-Run-05 实施口径

`ADB-Run-05` 的目标是验证 runtime 发行包脚本本身可运行：

- `:vexra-adb:test` 依赖 `:vexra-adb:adbRuntimeDist`，确保脚本级 smoke 使用最新 runtime zip。
- 新增测试解压 runtime zip 到临时目录，通过 `bin/adb-sql-server` 或 `bin/adb-sql-server.bat` 启动独立 SQL server 进程。
- 测试通过 `jdbc:adb:tcp://127.0.0.1:<port>/...` 完成 ADB 表建表、insert 和 select，并通过 stop 文件关闭脚本启动的进程。
- 本阶段只验证 SQL server 脚本级可运行性；region node 多进程可运行性继续由 `ADB-Run-02` 的产品 main class smoke 覆盖。

`ADB-Run-05` 已完成：

- `vexra-adb` 的 `test` 任务已依赖 `adbRuntimeDist`，确保脚本级 smoke 使用最新 runtime zip。
- 新增 `AdbRuntimeDistributionSmokeTest`，解压 runtime zip 后通过发行包内 `bin/adb-sql-server` 或 `bin/adb-sql-server.bat` 启动 SQL server。
- 该测试已通过 `jdbc:adb:tcp://127.0.0.1:<port>/...` 完成 ADB 表建表、insert、select 和 stop 文件关闭。

### ADB-Run-06 实施口径

`ADB-Run-06` 的目标是验证 runtime 发行包内 region node 脚本本身可运行：

- 新增测试解压 runtime zip 到临时目录，通过 `bin/adb-region-node` 或 `bin/adb-region-node.bat` 启动 3 个独立 region node 进程。
- 每个进程使用 `--group`、`--node`、`--peers`、`--host`、`--port`、`--storage`、`--cache`、`--ready` 和 `--stop` 参数，覆盖发行包脚本到 `AdbRegionNodeMain` 的完整参数链路。
- 测试通过真实 `RaftRClient`、`AdbRpcRegionCommitClient` 和 `AdbRaftRegionScanClient` 完成 prewrite、commit 和 region scan，并通过 stop 文件清理子进程。
- 本阶段不实现 SQL server 与 region node 的自动编排；它只证明发行包 region node 脚本可运行并可承载 Raft/GRPC 数据路径。
...
`ADB-Run-08` 把显式 opt-in 的 SQL 写入路径接到远端 Raft region node：

- table-engine 参数新增 `adb.distributed.write.client=raft` 和 `adb.distributed.write.timeoutMillis`；默认仍为本地写入，不改变旧 `jdbc:adb:*` 行为。
- `AdbSqlDistributedWriteRuntime` 复用 `AdbRpcRegionCommitClient` 和 `AdbRaftRegionCommitTransport`，把 SQL 表写入提交到 region node。
- `AdbRegionCommitCoordinator` 新增可选 key mapper，用于在 SQL server 与 region node 暂未共享 catalog 时，把本地 table id/epoch 显式映射到远端 table id/epoch。
- 对远端 Raft 写入，单 region 也强制执行 PREWRITE + COMMIT，避免原本给本地 bridge 使用的 single-region commit fast path 绕过 durable intent。
- `AdbSqlDistributedTimestampProvider` 作为临时 TSO bridge，保证固定 readTs 测试中 `startTs < commitTs < readTs`；真实共享 catalog/TSO 留给 `ADB-Run-09`。

`ADB-Run-08` 已完成：

- `AdbTableProvider` 在显式开启 raft write client 时安装 SQL distributed write runtime、timestamp provider 和 region commit coordinator。
- `AdbSqlServerRemoteRegionScanSmokeTest` 已升级为 forked SQL server 通过 SQL `INSERT` 写入远端 region node，再通过 SQL 远端 scan 和 direct Raft scan 读回。
- 覆盖测试包括 `AdbSqlDistributedScanConfigTest`、`AdbSqlDistributedTimestampProviderTest`、`AdbRegionCommitCoordinatorTest` 和 `AdbSqlServerRemoteRegionScanSmokeTest`。

### ADB-Run-09 实施口径

`ADB-Run-09` 把显式 table id、table epoch、Raft 目标和固定 readTs 收敛到共享 catalog/TSO 原型：

- 新增 UTF-8 properties 形式的共享 catalog 快照，包含 `adb.catalog.raft.*`、`adb.catalog.tso.*` 和 `adb.catalog.table.<name>.*`。
- table-engine 参数新增 `adb.distributed.catalog.path` 和 `adb.distributed.catalog.table`；未显式提供 table id、table epoch、Raft group、peers、dbName 或 readTs 时，自动从 catalog 快照补齐。
- `AdbTableProvider` 按当前 SQL 表名解析 catalog，使远端 SQL 读写建表不再依赖手工 `adb.distributed.table.id`、`adb.distributed.table.epoch` 和 `adb.distributed.scan.readTs`。

`ADB-Run-09` 已完成：

- `AdbSqlSharedCatalogSnapshot` 已提供共享 catalog/TSO 快照解析和表绑定校验。
- `AdbSqlDistributedScanConfigTest` 覆盖 catalog 参数补齐。
- `AdbSqlServerRemoteRegionScanSmokeTest` 已改为通过 catalog 文件完成 SQL server 到远端 region node 的写入和读取闭环。

### ADB-Run-10 实施口径

`ADB-Run-10` 把 SQL server 与 region node 的手工启动参数收敛到一份集群 properties 配置：

- 新增 `AdbClusterOrchestrationConfig`，解析 runtime 目录、SQL server、region node、Raft group、共享 catalog 路径和 catalog 表/TSO 信息。
- 新增 `AdbClusterOrchestrationPlan`，从一份配置生成 SQL server 命令、region node 命令、共享 catalog 文件和预检诊断。
- 新增 `AdbClusterPlanMain` 与 runtime 脚本 `adb-cluster-plan`，发行包可直接读取 `--config` 输出编排计划，并可用 `--writeCatalog true` 写出 catalog 文件。
...
`ADB-Run-12` 在已有长稳压测 evaluator 之外增加端到端集群门禁：

- 新增 `AdbEndToEndClusterStressReport`，把长稳压测报告、SQL/region 读写 smoke、恢复演练、滚动升级演练和 smoke 循环次数放到同一个发布证据对象。
- 新增 `AdbEndToEndClusterStressGate`，复用 `AdbLongRunStressEvaluator` 的失败率、P99、维护循环和故障矩阵校验，再额外要求集群读写、恢复演练、滚动升级演练全部通过。

`ADB-Run-12` 已完成：

- `AdbEndToEndClusterStressGateTest` 覆盖完整报告通过、集群读写失败、恢复/滚动升级缺失、底层长稳指标失败等门禁场景。
- 本阶段提供 release gate 模型和测试证据，不直接执行外部长时间压测平台；真实长跑数据可按该报告结构接入。

### ADB-Prod-03 当前进展

`ADB-Prod-03` 已按 h2db 当前开放的 table/index SPI 完成 SQL 路径真实接入，不要求 h2db 暴露 parser 或 optimizer 内部对象：

- h2db 当前插件指南仍明确 parser、optimizer 和 JDBC server 内部类不作为插件 API 暴露，因此本阶段不新增真正的 `EXPLAIN DISTRIBUTED` SQL 语法。
- ADB 表可通过 table engine `WITH` 参数显式开启分布式 SQL scan；未开启时保持现有单机 ADB/H2 行为。
- 开启后，`AdbPrimaryIndex.find(...)` 把 H2 传入的主键范围转换为 `DistributedPlan`，通过 `AdbDistributedRegionScanExecutor` 执行 region scan，再把 region 返回的 ADB row payload 还原为 H2 `Row`。
- 标准 JDBC `EXPLAIN SELECT ...` 通过 index plan SQL 输出 ADB distributed scan 标记，作为 `EXPLAIN DISTRIBUTED` 语法开放前的等价诊断。
- EXPLAIN 诊断输出最小 region 数统计；H2 仍通过现有 row count / index cost 接口进行计划选择，持久化统计和更复杂代价模型可作为后续优化，不阻塞本阶段验收。
...
`ADB-Prod-05` 的目标是把已有运行时门面组织成可执行的多节点部署与安全验收包：

- 新增 ADB 部署节点描述和部署计划，描述 nodeId、host、端口、数据目录、角色和安全材料位置，并生成可审计的启动命令清单。
- 部署计划必须复用 `AdbDistributedRuntimeOptions` 的安全门禁：启用分布式模式时必须开启 TLS 和最小权限；同时校验 nodeId、端口和数据目录不能冲突。
- 新增部署预检/演练器，基于控制面快照刷新 `AdbRClientRegistry`，检查 region leader client 是否可注册，并输出 operations system row/metrics 作为部署验收证据。
- 备份恢复演练继续复用 `AdbRuntimeOperationsBridge` 的 FULL backup/restore drill，本阶段只把它纳入部署验收流程，不扩展 PITR 或对象存储。
- 滚动升级演练复用 `RollingUpgradePlan`，逐节点标记升级完成，并在每一步要求集群仍有可写 region，避免把不可用集群标记为可升级。
- 本阶段不负责真实证书签发、操作系统服务安装、外部服务发现、真实远程连接池生命周期或长稳故障注入；这些继续归入 `ADB-Prod-06` 或部署系统集成。
- 阶段验收要求 JUnit 覆盖安全配置拒绝、启动清单生成、RClient registry 刷新、system row/metrics 暴露、backup/restore drill 和 rolling upgrade drill。

`ADB-Prod-05` 阶段验收状态：

- 本阶段已完成。阶段验收由 `AdbDeploymentPlanTest`、`AdbDeploymentDrillTest` 和 `AdbRuntimeOperationsBridgeTest` 覆盖。
...
- 本增量不引入完整 filter/projection proto；复杂 SQL pushdown 和更细代价选择在 `ADB-Prod-03` 验收后作为后续优化继续推进。
...
本轮 `ADB-Prod-02` 的本地 safe point 持久化/租约口径：

- 新增 `AdbSafePointLeaseStore` 和 `AdbSafePointLeaseRecord`，把全局 safe point、lease owner 和 lease 到期时间编码持久化到 META CF，作为后续控制面复制或 system table 暴露的本地边界。
