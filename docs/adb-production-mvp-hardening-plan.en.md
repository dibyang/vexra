...
- This increment covers node heartbeat records, region snapshots, route epoch, route watch, control-plane TTL write protection, monotonic TSO, and `AdbSystemTableProvider` output for nodes/regions/tso. Leases, replicated control-plane service, and lease/capability system tables remain later GA-03 subtasks.
