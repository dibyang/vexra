...
- This increment covers node heartbeat records, region snapshots, route epoch, route watch, control-plane TTL write protection, monotonic TSO, and `AdbSystemTableProvider` output for nodes/regions/tso/leases/capabilities. Replicated control-plane service and strong binding between leases and the real TSO owner remain later GA-03 subtasks.
...
- Added `AdbSqlDiagnosticEvent`, `AdbSqlDiagnosticRecorder`, and `AdbSqlDiagnosticSnapshot` as an embeddable SQL diagnostics API. It records total SQL count, slow SQL count, failed SQL count, max latency, recent slow SQL, and recent failed SQL, and can export diagnostic-bundle operations / metrics fields.
- `adbRuntimeDist` now includes the `adb-doctor` script. `AdbDiagnosticBundleWriterTest` covers redaction and SQL-summary output, `AdbDiagnosticLogTailerTest` covers log tailing, `AdbDiagnosticPropertiesCollectorTest` covers evidence/report collection, `AdbRuntimeDiagnosticCollectorTest` covers runtime system row / metrics collection, `AdbSqlDiagnosticRecorderTest` covers slow/failed SQL summaries, `AdbDoctorMainTest` covers bundle generation through the main method, and `AdbRuntimeDistributionSmokeTest` covers the runtime zip script.
- Later GA-06 work still needs to wire the SQL diagnostics recorder into h2db plugin listeners or JDBC/Server hooks, add automatic log discovery and latest in-memory lock resolve / GC worker results, and design live runtime connection parameters for `adb-doctor`.
