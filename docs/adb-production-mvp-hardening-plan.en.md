...
- `adbRuntimeDist` now includes the `adb-doctor` script. `AdbDiagnosticBundleWriterTest` covers redaction, SQL-summary output, and background-worker summary output; `AdbDiagnosticLogTailerTest` covers log tailing; `AdbDiagnosticPropertiesCollectorTest` covers evidence/report collection; `AdbRuntimeDiagnosticCollectorTest` covers runtime system row / metrics collection; `AdbSqlDiagnosticRecorderTest` covers slow/failed SQL summaries; `AdbBackgroundWorkerDiagnosticCollectorTest` covers lock resolve / GC worker last-result collection; `AdbDoctorMainTest` covers bundle generation through the main method; and `AdbRuntimeDistributionSmokeTest` covers the runtime zip script.
- Later GA-06 work still needs to wire the SQL diagnostics recorder into h2db plugin listeners or JDBC/Server hooks, add automatic log discovery, and design live runtime connection parameters for `adb-doctor`.
