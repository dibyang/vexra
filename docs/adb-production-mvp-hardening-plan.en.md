...
- `adbRuntimeDist` now includes the `adb-doctor` script. `AdbDiagnosticBundleWriterTest` covers redaction, SQL-summary output, and background-worker summary output; `AdbDiagnosticLogTailerTest` covers log tailing; `AdbDiagnosticLogDiscovererTest` covers automatic log discovery; `AdbDiagnosticPropertiesCollectorTest` covers evidence/report collection; `AdbRuntimeDiagnosticCollectorTest` covers runtime system row / metrics and SQL registry aggregate collection; `AdbSqlDiagnosticRecorderTest` covers slow/failed SQL summaries; `AdbTableProviderIntegrationTest` covers JDBC table-engine paths automatically producing SQL diagnostic events; `AdbBackgroundWorkerDiagnosticCollectorTest` covers lock resolve / GC worker last-result collection; `AdbDoctorMainTest` covers bundle generation, live runtime parameter output, and missing-endpoint validation; and `AdbRuntimeDistributionSmokeTest` covers the runtime zip script.
