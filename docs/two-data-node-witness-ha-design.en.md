...
| HA-06 | Done | `HaStatusCollector` and `HaStatusSnapshot` expose HA mode, leader, epoch, term, voter count, reachable voters, quorum threshold, witness presence, writable status, failover status, system-table rows, and numeric metrics fields. |
