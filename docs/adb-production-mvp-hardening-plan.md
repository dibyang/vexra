...
- `adb-doctor` 支持 `--logs path1,path2`、`--autoLogs true`、`--maxAutoLogFiles n` 与 `--logTailLines n` 采集关键日志尾部；自动发现会基于 runtime logs、SQL baseDir 和节点 dataDir 做浅层扫描，日志文件缺失时写入缺失说明而不让整个诊断包失败。
...
- 新增 `AdbSqlDiagnosticEvent`、`AdbSqlDiagnosticRecorder`、`AdbSqlDiagnosticSnapshot` 和 `AdbSqlDiagnosticsRegistry`，提供可嵌入的 SQL 诊断 API，记录累计 SQL 数、慢 SQL 数、失败 SQL 数、最大耗时、最近慢 SQL 和最近失败 SQL，并可转换为诊断包 operations / metrics 字段。
- SQL 诊断记录器已接入 `AdbTableProvider` 创建的 `TxnManager`，并在 `AdbTable`、`AdbPrimaryIndex`、`AdbSecondaryIndex` 的真实 table/index 操作入口记录 `ADB_TABLE_*` 摘要。当前插件层拿不到 h2db 原始 SQL 文本，因此先记录 SQL 类型、表名、操作名、耗时和异常；后续 h2db 若开放 statement listener/JDBC hook，再补充原始 SQL 文本。
- `AdbRuntimeDiagnosticCollector` 已合并 `AdbSqlDiagnosticsRegistry.snapshotAll()`，诊断包 operations 可以看到每个 SQL scope 的最近慢 SQL/失败 SQL 摘要，metrics 可以看到聚合 SQL scope 数、SQL 总数、慢 SQL 数、失败 SQL 数和最大耗时。
- `adb-doctor` 已支持 live runtime 诊断参数：`--liveRuntime`、`--runtimeHost`、`--runtimePort`、`--runtimeTimeoutMillis` 和 `--runtimeTls`。当前实现只把连接意图、endpoint、timeout、TLS 标志和 `endpoint_not_implemented` 状态写入诊断包，不主动连接远程 runtime；后续 runtime 管理端点定稿后在同一输出字段下替换为真实采集结果。
...
- 新增 `AdbObservabilityProductionReport` 和 `AdbObservabilityProductionGate`，把诊断包、敏感配置脱敏、日志尾部、操作报告、runtime metrics、system table、SQL 诊断、后台任务诊断和 live runtime 端点状态汇总为 GA-06 结构化发布门禁输入。门禁只校验证据完整性；真实证据仍由 doctor、runtime collector、system table smoke、SQL 诊断记录器和后续运维系统生成。
- `adbRuntimeDist` 已包含 `adb-doctor` 脚本；`AdbDiagnosticBundleWriterTest` 覆盖脱敏输出、SQL 摘要和后台 worker 摘要写出，`AdbDiagnosticLogTailerTest` 覆盖日志 tail，`AdbDiagnosticLogDiscovererTest` 覆盖自动日志发现，`AdbDiagnosticPropertiesCollectorTest` 覆盖 evidence/report 采集，`AdbRuntimeDiagnosticCollectorTest` 覆盖 runtime system row/metrics 与 SQL registry 聚合采集，`AdbSqlDiagnosticRecorderTest` 覆盖慢 SQL/失败 SQL 摘要，`AdbTableProviderIntegrationTest` 覆盖 JDBC table engine 路径自动产生 SQL 诊断事件，`AdbBackgroundWorkerDiagnosticCollectorTest` 覆盖 lock resolve / GC worker 最近结果采集，`AdbDoctorMainTest` 覆盖 main 方法生成诊断包、live runtime 参数输出和缺失 endpoint 校验，`AdbRuntimeDistributionSmokeTest` 覆盖 runtime zip 中的 doctor 脚本，`AdbObservabilityProductionGateTest` 覆盖完整通过以及诊断包/脱敏/日志、runtime/system table/SQL/后台任务、操作报告/live runtime 状态缺失时的拒绝路径。
- 后续 GA-06 仍需实现真实 runtime 管理端点采集，并在 h2db 暴露更高层 statement/JDBC hook 后补充原始 SQL 文本采集。
